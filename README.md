...
A study of Sentinel-2 land cover rather than a single model. A classical Random-Forest classifier is kept as a baseline and compared against a U-Net and a LoRA-fine-tuned geospatial foundation model on identical, spatially blocked splits, plus a DINOv3 linear probe on EuroSAT and label-efficiency curves. The question it sets out to answer: when does the classical method win, and when does the extra model capacity earn its cost?

The classical core is fully tested and CPU-only: spectral indices, a five-class scheme with a unit-tested ESA-WorldCover remap, per-pixel and per-patch Random Forests, shared metrics, and spatially blocked splits. The deep and foundation-model tracks run through headless GPU scripts that import the same tested package, so the comparison is fair by construction and the only untested surface is the training call itself.
...
When does the classical method win? The two tracks answer in opposite directions, which is what makes the comparison useful. On EuroSAT, with 27 000 clean labelled patches, model capacity is rewarded and the ordering is Random Forest 0.885 < fine-tuned ResNet-18 0.974 < DINOv3-sat linear probe 0.987. The last result is the interesting one: a frozen satellite-pretrained self-supervised ViT with nothing but a logistic-regression head on top beats a fully fine-tuned CNN, which shows why EO foundation-model representations are worth their size. On the Sentinel-2 segmentation track, with one small Thessaloniki AOI and only a handful of WorldCover-supervised chips, the direction flips. The per-pixel Random Forest beats the U-Net (mean IoU 0.55 vs 0.45), because a data-hungry segmentation network cannot out-learn the RF's spectral separability from so few labels. The Prithvi-EO-2.0 foundation model, LoRA-fine-tuned with only 6.7 % of its 326 M parameters trainable, lands lowest of the three here (mean IoU 0.18). That is honest and unsurprising: adapting a 300 M geospatial transformer on ~6 chips is exactly the regime where a big model has too little signal, and its label efficiency would only show at scale. The contrast is the reproducible point of the repo. Capacity and pretrained representations win when labels are plentiful (EuroSAT), and the ~100x cheaper classical baseline wins in the low-label regime (this AOI). The label-efficiency curve is noisy at this dataset size, so read the 10 % and 100 % endpoints rather than the wiggle in between; a larger AOI would lift both the U-Net and the foundation model. Every one of these caveats is stated rather than hidden.
...
Both tracks are measured end-to-end on real data and complete. Track 1 (EuroSAT MS) compares RF, ResNet-18 and the DINOv3-sat probe; Track 2 compares a per-pixel RF, a U-Net, and a LoRA-fine-tuned Prithvi-EO-2.0 on a real Sentinel-2 composite plus ESA WorldCover over Thessaloniki. `landcover predict` produces a classified GeoTIFF with a preserved CRS/transform and per-class area statistics, the artifact a downstream geospatial agent wraps as a tool.
...
- **Single small AOI, few chips.** Track 2 uses one cloud-masked median composite over one Thessaloniki AOI (≈12 chips); this deliberately small-label regime is what lets the Random Forest out-score the U-Net, and it makes the label-efficiency curve noisy. A wider AOI would grow the chip count and shift the balance toward the deep models, and the `--aoi` flag takes any bounding box.
